...
export default function ReadingRestructurer() {
  ...
  const [sessionMetrics, setSessionMetrics] = useState({
    replayRate: 0,
    timeOnChunk: 1.0, // Ratio relative to baseline
    dropPoint: 0.0,
    vocabFriction: 0,
    
    // Internal counters
    replays: {}, // { chunkId: count }
    lastStartTime: null,
    totalDuration: 0,
    chunkCount: 0,
  ...
  const handleChunkClick = (chunkId) => {
    if (activeChunkId === chunkId) {
      // Already active? Do nothing on single click to avoid reset
      return
    }
  ...
  function speakChunk(chunkId, text, lang, ttsUrl) {
    ...
    const speed = output?.metadata?.playback_speed || 1.0
  ...
  async function runRestructure(event) {
  ...
  function renderChunkText(chunk) {
    ...
    return parts.map((part, index) => {
      ...
      const displayWord = isAugmented ? syllableChunk(part.replace(/[-·]/g, '')) : part

      return (
        <span
          key={`${chunk.chunk_id}-part-${index}`}
          className={`clickable-word ${isAugmented ? 'user-augmented' : ''} ${isHighlighted ? 'term-highlight' : ''}`}
  ...
  function renderClickableText(text) {
    ...
    return parts.map((part, index) => {
      ...
      const className = `clickable-word ${isAugmented ? 'user-augmented' : ''}`
    ...
    <main className="app-shell">
      <section className="panel">
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '0.75rem' }}>
          <span className="simple-label" style={{ background: modelMode === 'local-mt5' ? '#dcfce7' : '#f1f5f9', color: modelMode === 'local-mt5' ? '#166534' : '#475569' }}>
            Model: {modelMode || 'loading...'}
          </span>
        </div>
        ...
        {output?.diagnostic_log && (
          <div className="diagnostic-card" style={{ marginTop: '1rem' }}>
            ...
            <div className="metrics-grid">
              {Object.entries(output.diagnostic_log.metrics || {}).map(([key, metric]) => (
            ...
            <div className="levers-section">
              ...
              <div className="lever-row">
                <div className="lever-detail">
                  ...
                  <span>Aggressiveness: <strong>{output.metadata?.levers?.aggressiveness}</strong></span>
                </div>
                <div className="lever-detail">
                  ...
                  <span>Playback Speed: <strong>{output.metadata?.levers?.playback_speed}</strong></span>
                </div>
              </div>
              <div className="lever-row" style={{ marginTop: '0.5rem' }}>
                <div className="lever-detail">
                  ...
                  <span>Chunk Size: <strong>{output.metadata?.levers?.chunk_size}</strong></span>
                </div>
              </div>
            </div>
          </div>
        ...
        {output && (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
            <div style={{ display: 'flex', gap: '0.8rem' }}>
              <span className="simple-label" style={{ background: '#f1f5f9', color: '#475569' }}>
                {output.metadata?.mode?.toUpperCase() || 'STABLE-CORE'}
              </span>
              <span className="simple-label" style={{ background: '#dcfce7', color: '#166534' }}>SIMPLIFIED</span>
            </div>
          </div>
        )}
      </section>
